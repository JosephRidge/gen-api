...
def receive_generator_data(request):
    # Define expected fields, multipliers, and enums
    field_map = [
    ...
    if len(data_array) != len(field_map):
        return Response({
            'error': f'Expected {len(field_map)} values, received {len(data_array)}.'
    ...
    try:
        # Map each field
        for idx, (field_name, multiplier, enum_dict) in enumerate(field_map):
            raw_value = data_array[idx]

            # Try to convert raw input to float and apply multiplier
            try:
                val = float(raw_value) * multiplier
            except (ValueError, TypeError):
                val = raw_value  # fall back to raw if not numeric

            # Apply enum mapping if present
            if enum_dict is not None:
                try:
                    val = enum_dict.get(int(raw_value), val)
                except (ValueError, TypeError):
                    pass

            mapped_data[field_name] = val
